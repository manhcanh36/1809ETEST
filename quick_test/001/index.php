...
if (isset($_GET['run']) && ($_GET['run'] == 1)) {
    echo '<br> Xem Lại';
    $path = __FILE__;
    $dir = dirname($path); //đường dẫn thư mục cha
    $target = $dir.'/demo'; //tạo thư mục demo dựa trên thư mục cha
    if (is_dir($target)) {
        for ($i = 1; $i < 101; $i++) {
            if ($i < 10){
                $name = '00'.$i;
            ...
            if (!file_exists($target_subdir)) {
                mkdir($target_subdir, 0777, true); //sẽ tạo mới thư mục theo đúng đường dẫn truyền vào.
                $target_subdir_file = $target_subdir.'/index.php';
                $fp=fopen($target_subdir_file,'w'); //sẽ mở thư mục dựa vào đường dẫn truyền vào
